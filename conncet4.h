...
void addrefrence(int x , int position)
{
	if (index [position] < 6 )
	{  
		if (counter % 2 == 0) { glColor3ub(255, 255, 0); player = 1;} 
		else {glColor3ub(150, 30, 30); player = 2;}
		switch (player)
		     {  
			     case 1: connect4 [index[position]][position] = 1 ; break;
			     case 2: connect4 [index[position]][position] = 2 ; break;	
			     default : break;
		     }
		...
		counter++;
		if (winning){set = false ; glutTimerFunc(2500,Timer,1);}
